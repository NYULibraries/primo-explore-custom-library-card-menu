angular
  ...
  .component('prmLibraryCardMenuAfter', {
    ...
    template: `<button ng-repeat="item in customLibraryCardMenuItems"
                       aria-label="{{ translate(item.description) }}"
                       data-href="{{ translate(item.action) }}"
                       ng-click="goToUrl(translate(item.action))"
